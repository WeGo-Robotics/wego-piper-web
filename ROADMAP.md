...
| **3b-6** | systemd 유닛화 | ☑ 학습·정책서버·xferd — **재부착 실기 확인** |
...
1. **학습 진행률 파서 × 재부착** — 유닛 재부착은 확인했지만(가짜 작업으로 `step 1/120` 부터
   이어 읽음), **실제 학습 로그를 파서가 제대로 먹는지는 안 봤다.** `restore()` 가
   `total_steps=0` 을 돌려주고 파서가 journald 를 다시 읽으며 채우는 설계인데, 그 경로가
   미검증이다. 학습을 걸고 게이트웨이를 재시작해 진행률이 이어지는지 보면 닫힌다
