...
**원격 추론이 처음으로 돌았다.** `.42` 정책 서버 + 클라이언트, gRPC 로 790스텝 / 40초,
실제 **19.90fps**(목표 20). 그 전까지 이 경로는 **한 번도 돈 적이 없었다** —
`main()` 이 `_paused` 를 `global` 선언 없이 대입해 제어 루프 첫 줄에서 죽고 있었다
(`Total steps: 0`). 돌려봐야만 드러나는 종류라 이제 정적 테스트가 막는다.
