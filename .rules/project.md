...
│   │   ├── routers/          # API 라우터 모듈
│   │   ├── services/         # 비즈니스 로직 (프로세스 관리, ZMQ 등)
│   │   ├── models/           # Pydantic 스키마
│   │   └── core/             # 설정, 상수, 유틸리티
│   ├── requirements.txt
│   └── pyproject.toml
...
│   ├── package.json
│   └── vite.config.ts
├── wrapper/                  # LeRobot 래퍼 (ZMQ 수신 + CLI 실행)
│   └── lerobot_wrapper.py
...
- **백엔드**: Python 3.11+, FastAPI, uvicorn, pyzmq, evdev
- **프론트엔드**: TypeScript, React, Vite, Tailwind CSS
- **IPC**: ZMQ (PUSH/PULL) — `tcp://127.0.0.1:5555`
...
- 한글 주석 허용. 변수/함수명은 영문
- `.env`로 환경별 설정 관리 (포트, ZMQ 주소, 카메라 디바이스 경로 등)
