...
# ─────────────────────────────────────────────────────────────────────────────
# ⚠ 학습이 안 되는 이미지로는 **빌리기 전에** 막는다
# ─────────────────────────────────────────────────────────────────────────────

def test_the_slim_image_is_marked_as_untrainable():
    """⚠ **실측(2026-09-17)**: slim 이미지(1.32GB)에는 lerobot 이 아예 없다 —
    `lerobot_policy_act_aux` 뿐이다. 그걸로 빌리면 기계를 받고 이미지를 내려받은
    **뒤에** 3초 만에 `ModuleNotFoundError` 로 죽는다. 실제로 그렇게 태웠다($0.0145).
    """
    from app.services.cloud.providers.vast import parse_template

    assert parse_template({"name": "piper-train slim cu126"}).can_train is False
    assert parse_template({"name": "piper-train full cu126"}).can_train is True
    # ⚠ 모르는 변종은 막지 않는다 — 사용자가 만든 템플릿까지 못 쓰게 된다
    assert parse_template({"name": "내가 만든 것"}).can_train is True


def test_renting_with_an_untrainable_template_is_refused_before_spending(monkeypatch):
    """⚠ 돈이 나가기 **전에** 아는 것과 나간 뒤에 아는 것의 차이다."""
    from fastapi.testclient import TestClient
    ...
    class _P:
        def templates(self):
            return [Template(id=1, name="piper-train slim cu126", image="i", tag="slim",
                             hash_id="SLIM", disk_gb=20, description="", variant="slim",
                             can_train=False)]

        def create(self, *a, **kw):                                  # pragma: no cover
            rented.append(a)
            raise AssertionError("학습 안 되는 템플릿으로 기계를 빌렸다")

    monkeypatch.setattr(router, "_provider", lambda: _P())
    r = TestClient(app).post("/api/cloud/rent", json={
        "offer_id": 1, "template_hash": "SLIM",
        "dataset_repo_id": "me/d", "policy_repo_id": "me/p"})
    assert r.status_code == 400 and "lerobot" in r.json()["detail"]
    assert rented == []


def test_a_template_lookup_failure_does_not_block_renting(monkeypatch):
    """⚠ Vast 가 안 닿는다고 빌리기를 막으면, 쓸 수 있는 템플릿까지 못 쓰게 된다 —
    막는 것은 "안 되는 것을 **확인했을 때**" 뿐이다."""
    from app.routers import cloud as router

    class _P:
        def templates(self):
            raise RuntimeError("Vast 안 닿음")

    monkeypatch.setattr(router, "_provider", lambda: _P())
    asyncio.run(router._reject_untrainable("whatever"))   # 예외가 나면 실패한다
