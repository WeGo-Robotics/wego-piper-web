...
def test_a_heartbeat_without_a_body_still_works():
    ...
    info = HeartbeatInfo()
    assert info.gap is None and info.hidden is None
...
def test_a_widening_gap_is_logged_before_it_trips(bridge, caplog, monkeypatch):
    ...
    with caplog.at_level("WARNING"):
        bridge.heartbeat()
        t[0] += 1.5
        bridge.heartbeat({"gap": 1480, "hidden": True})

    assert "heartbeat 간격 1.50s" in caplog.text
    # 브라우저가 잰 값과 탭 상태가 같이 남아야 어디서 벌어졌는지 갈린다
    assert "1480" in caplog.text and "hidden=True" in caplog.text
...
def test_the_browser_reports_its_own_gap_and_visibility():
    ...
    assert "'/estop/heartbeat', { gap, hidden" in src.replace('"', "'"), \
