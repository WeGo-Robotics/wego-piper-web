...
⚠ 이 파일이 지키는 것은 **비밀이 화면으로 안 나가는 것**이다. 게이트웨이 로그인은
켜야 걸리고(설정 → 보안) 기본은 꺼져 있다 — 꺼져 있으면 LAN 에서 이 포트에 닿는 누구나
응답을 읽는다. 한 번 돌려주는 순간 그 응답이 곧 유출 경로다.

⚠ 로그인을 켜도 이 규칙은 그대로다. 인증은 **누가** 보느냐를 좁힐 뿐이고, 비밀을 응답에
싣는 것은 로그·프록시·브라우저 기록에도 남긴다는 뜻이다.
"""
