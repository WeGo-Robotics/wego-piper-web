...
def test_the_list_covers_every_mutating_post():
    ...
    known = {p for p, _ in _MUST_PERSIST} | {
        ...
        "/safety",
        # 부하 감시 임계 — 바닥 필터와 같은 자리다. robotd 가 자기 `load.json`
        # 에 **팔별로** 저장한다(팔마다 슬립이 잦은 관절이 다르다). 세션에
        ...
        # 거치 방향(180°) — 좌/우와 같은 자리다. 그 **기계**의 물리적 사실이라
        # 게이트웨이 세션이 아니라 so101d 세션(by_id 별)에 남는다: 어댑터를
        # 뽑아 다른 PC 에 꽂아도 돌려 놓았다는 사실은 그 팔을 따라간다.
        "/serial/flipped",
        ...
        "/reset",
        # 버스 초기화 — 인터페이스를 내렸다 올리는 일회성 조작이다. 세션에
        # 실으면 재연결 때마다 버스가 끊긴다.
        "/bus/reset",
        # 검사 중심 오프셋 — 팔에 붙은 기구물의 성질이지 세션이 아니다.
        # 팔을 다시 등록해도 기구물은 그대로 달려 있다.
        "/diag/offsets",
        # 검사 결과 저장 — 세션(팔 슬롯·역할·config)이 아니라 별도 보관함으로
        # 간다. 팔의 구성이 아니라 그때의 측정이다.
        "/diag/save",
        # 관절 검사 — 팔을 흔들어 재는 일회성 조작이다. 세션에 실을 것이 없고,
        # 실었다면 재연결 때마다 팔이 저절로 흔들린다.
        "/diag/start", "/diag/stop",
        # 전체 초기화 — 세션에 **쓰는** 게 아니라 파일을 지운다. 그래서 여기
        # 규칙("바꿨으면 save_session 해라")의 반대편이고, 지웠는지는
        # tests/test_robot_clear.py 가 따로 지킨다.
        "/clear",
