...
⚠ **왜 `ssh-keygen` 이 아니라 라이브러리인가.** 이미지에 `openssh-client` 가 없다
(실측: `piper-web-backend:latest` 안에 `ssh-keygen` 없음). 서브프로세스로 만들면
같은 함정에 다시 빠진다. 그래서 `cryptography` 로 **프로세스 안에서** 만든다 —
덤으로 테스트가 바이너리 없이 돈다.
