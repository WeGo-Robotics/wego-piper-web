...
#: 자세 프리셋. 검사와 **따로** 둔다.
#:
#: ⚠ 한 자세를 여러 검사가 쓴다 — 같은 자세에서 손목 카메라와 탑 카메라를 각각
#:   보는 것이 흔한 배치다(feature 의 두 배치 표). 검사 안에 자세를 묻어 두면
#:   같은 자세를 두 번 만들게 되고, 둘이 조금씩 어긋나면 두 측정이 비교가 안 된다.
POSE_DOMAIN = "alignment_pose"
