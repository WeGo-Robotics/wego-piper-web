...
class RecordStartRequest(BaseModel):
    ...
    camera_height: int = 0
    camera_fps: int = 0
    # 데이터셋 제목·설명 — 정지 시 meta/piper_notes.json 사이드카로 남는다
    # (제목 = 사이드카의 name). LeRobot info.json 에는 이 자리가 없다 (notes_sidecar 참고).
    title: str = ""
    description: str = ""
...
async def start_recording(body: RecordStartRequest):
    ...
    # ⚠ 시뮬로 모으면 **어느 세계에서 모았는지**가 데이터의 일부다. 가상환경을 여러 개 만들기
    #   시작하면 그 뒤에 모은 에피소드는 구분이 안 되고, **나중에 붙여도 소급이 안 된다**.
    #   교체는 수집 중엔 막혀 있으므로(exclusivity.SCENE_SWAP) 시작 때의 가상환경이 곧 끝의 가상환경이다.
    ...
    if any(str(pt or "").startswith("sim_") for pt in [body.robot_port, *body.robot_ports]):
        try:
            ...
            logger.warning("가상환경을 못 읽었다 (사이드카 생략): %s", exc)
...
async def stop_recording():
    ...
    # 제목·설명도 같은 시점에 사이드카로. 둘 다 비어 있으면 안 쓴다 — 기존 값을
    # 빈 값으로 덮는 사고를 막는다. 실패해도 정지 흐름은 막지 않는다 (카메라 사이드카와 동일).
    desc = (_last_recording.get("description") or "").strip()
    title = (_last_recording.get("title") or "").strip()
    if repo_id and (title or desc):
        try:
    ...
    # 가상환경도 같은 시점에. 실기로 모았으면 아무것도 안 쓴다 — 없는 것이 정상이다.
