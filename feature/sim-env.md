...
4. ◐ **스크립트 시연 → 수집 → 학습 → 추론 → 자동 판정** — E2E 한 바퀴.
   - ☑ **시연** (2026-09-17) — `backend/app/services/sim_demo.py` + `/api/sim/demo`.
     큐브를 집어 통에 넣는 한 바퀴를 **리더 세그먼트(`sim_leader1`)로 발행**한다. 웹 리더와
     같은 자리라 수집·추론·E-stop 경로가 하나도 안 바뀐다(수집 중엔 `relay=False` 로 발행만).
     ...
     것이 맞다.
   - ☐ 수집 배선 — 수집 화면에서 리더로 `sim_leader1` 을 고를 수 있어야 한다
     (`teleop_port`). 지금은 API 로만 된다.
