...
3. ✅ **등록 UI** (2026-09-09) — `SimArmInfo(ArmInfo)`: RPC 를 타는 표면 전부를
   ...
   떠 있는 그리퍼(장면의 <option>·손가락·큐브 문자열 그대로)로 내려가 닫고 들어 흔드는
...
4. ◐ **스크립트 시연 → 수집 → 학습 → 추론 → 자동 판정** — E2E 한 바퀴.
   - ☑ **시연** (2026-09-17) — `backend/app/services/sim_demo.py` + `/api/sim/demo`.
     ...
     장면에서도 돈다. 시작 전에 궤적 전체를 풀어 보고 못 닿으면 거절한다(에피소드를 버리지
   ...
   - ☑ **수집 화면의 시연 버튼** (2026-09-17) — 리더를 `게이트웨이 리더` 로 고르고 시뮬 팔을
     ...
     화면이 그 어긋남을 경고한다.
   - ☐ 학습 → 시뮬 추론 → `/eval/stats` 성공률 (장면별로 갈린다 — sim-scene-editor 4단계).
