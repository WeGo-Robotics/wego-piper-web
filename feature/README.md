...
| 정책 UI 스펙 | 모델별 화면 항목을 YAML 로 선언하고 UI 가 그걸 읽어 동적 구성. 학습 필드·프로브 분기가 지금 TSX 안에 있다 | 구조 개편과 **안 겹침** — `core/policies.py` 를 dict 에서 로더로 바꾸는 일이라 공개 함수는 그대로 | [policy-ui-spec.md](policy-ui-spec.md) |
