...
> **✓ 1~4단계 구현 완료.** 캡처 3경로(라이브 shm·에피소드 캐시 복사·동영상
> canvas/외부 업로드, d2ff0c7) + 브라우저 라벨러·일괄 사전 라벨(44b7a15) +
> `piper-yolotrain` 유닛(그룹 단위 val 분할·유닛 내 수확·곁 JSON, 42affe8) +
> 루프 마감(곁 JSON→드롭다운 설명, 학습 완료→`/yolo-demo?model=` 단축).
> 실기 검증: 캡처 8장→사전 라벨→2에폭 학습→데모 드롭다운 등장까지 왕복.
> **남은 것**: 외부 라벨링 도구용 zip 가져오기/내보내기 (탈출구, 미착수).
