...
### 2.5 일반화 — 관측에 "상태 필드"를 넣을 때의 공급 규칙

§2.3~2.4의 결정을 규칙으로 적어둔다. **관측에 넣은 필드는 추론 때도 매 스텝 누군가
채워야 한다** — 모델에게는 조건이지, 스스로 아는 값이 아니다. 공급자는 셋뿐이고,
아래가 판단 순서이기도 하다:

| 공급자 | 성립 조건 | 예 |
|---|---|---|
| ① 센서 실측 | 읽으면 되는 값 | 관절각. 로드셀 관측화([demo-scenario-gaps G3](demo-scenario-gaps.md)의 hard 쪽)도 여기 |
| ② 온라인 추정기 | 관측에서 **인과적으로**(미래를 안 보고) 계산 가능. 라벨 생성기와 추정기가 **같은 코드**여야 분포가 안 어긋난다 | 페이즈 — 이 문서 전체 |
| ③ 외부 주입 | 관측에서 도출 불가, 사람/오케스트레이터가 아는 값 | `task` 지시문, [episode-orchestrator](episode-orchestrator.md)의 시나리오 상태 |

**④ "정책 예측 되먹임(재귀)"은 목록에 없다.** §2.3의 기각 이유 — 액션 경로 전부에서
잘라내야 하고, 되먹임 오류에 회복 경로가 없다 — 는 페이즈 특유의 사정이 아니라
모든 상태 필드에 그대로 적용된다. 앞으로 어떤 필드를 추가하든 ①→②→③ 순서로
공급자를 먼저 정하고, **셋 다 없으면 그 필드는 관측에 넣지 않는다.**
