...
#### ⚠ 남은 낭비: slim 은 torch 를 **두 번** 받는다

`install-stack.sh` 는 lerobot 을 먼저 깔고(그때 PyPI 기본 CUDA 빌드 torch 가 딸려 온다)
그걸 지운 뒤 원하는 CUDA 빌드로 다시 깐다. 이유는 파일 머리말에 있다(lerobot 0.5.0 은
torch<2.11 을 요구하고 `--no-deps` 로 깔면 nvidia 런타임이 빠진다). 결과적으로 부팅마다
5~6GB 를 받는다 — 실측 201초 중 상당 부분이 그것이다. 고치면 slim 이 더 빨라지지만
스택 설치를 건드리는 일이라 따로 잡는다.
