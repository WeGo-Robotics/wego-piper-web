...
| **컨테이너 배포판에 `ssh` 가 없다** | ☑ (베이스 `cu130-4`, §12-27) | 개발 머신은 게이트웨이가 저장소에서 직접 돌아 호스트의 ssh 를 쓸 뿐이었다. ⚠ **`tmux` 는 여기 낄 것이 아니었다** — `command -v tmux` 는 ssh 로 *원격에서* 돈다(`runners/ssh.py`). 게이트웨이에 필요한 것은 `ssh`·`scp` 클라이언트뿐이고, `vastai` 는 그새 들어갔다 — §3-4 · §12-27 |
| 인스턴스 조달·파기 (수명 관리) | ☐ | §3 |
...
4. **컨테이너에 ssh 를 넣는다** — 새로 드러난 전제. 배포판 게이트웨이는 컨테이너(root)라
   ...
   - ☑ `openssh-client` 를 [Dockerfile.base](../backend/Dockerfile.base) 에 (베이스 태그
     **`cu130-4`**, `backend/BASE_VERSION` 과 같이). `vastai` 는 회사 코드 옆 앱 이미지에 pip.
     ⚠ apt 목록(39행) 이 아니라 **파일 끝**에 붙였다 — 앞쪽에 끼우면 뒤의 torch·lerobot
     계층이 전부 새 다이제스트가 되어 호스트가 수 GB 를 다시 받는다(§12-27).
...
| ☑ `openssh-client` (베이스 `cu130-4` — §12-27 에서야 실제로 들어갔다), `vastai` pip (앱 이미지) | `Dockerfile.base`, `BASE_VERSION`, `Dockerfile` |
...
#### 받는 자리를 `cache_dir` 로 못 박았다 (→ §12-26 에서 다시 바뀜)
...
⚠ 이 판단은 "스캐너가 읽느냐" 만 봤고 **"로컬 학습과 같은 모양이냐" 는 안 봤다.** 그
대가는 §12-26 에 있다 — 읽히기는 하는데 한 학습이 두 덩어리로 갈라졌다.
...
#### ⚠ 저장한 키가 **일부 호출에만** 가고 있었다

키를 화면으로 넣었는데 SSH 등록 확인은 계속 `401 Invalid user key` 였다. 원인은 라우터가
`vastai` 를 **맨 환경으로** 띄운 자리가 둘 있었다는 것이다:

| 호출 | 무엇 | 결과 |
|---|---|---|
| `_provider()` 경유 | 잔액·오퍼·인스턴스 | ☑ 저장한 키가 env 로 간다 |
| `create ssh-key` | **[Vast 계정에 등록] 버튼** | ✗ 맨 환경 — CLI 의 제 파일(낡은 키)을 본다 |
| `show ssh-keys` | 등록 확인 | ✗ 같음 |

그래서 `show user` 는 통과하는데 `show ssh-keys` 만 401 이었다 — §12-23 에서 "API 키의
권한 문제" 로 읽었던 것이 사실은 **키가 그 호출에 안 실린 것**이었다.

⚠ **등록 버튼까지 그랬다는 게 더 나쁘다.** 눌러도 그 계정에 안 갈 수 있었다 — 사람은
눌렀다고 믿는데.

`VastProvider.env()` 를 공개로 두고 두 자리가 그것을 쓴다. 고친 뒤 실기에서
`registered: True` 가 떴다 — 계정에서 같은 지문을 실제로 찾았다는 뜻이고, 그전까지는
찾아본 적조차 없었다.
...
### 12-24. 기다리는 동안 무엇을 기다리는지 말한다 (2026-09-18)

빌려 둔 기계에 학습을 걸면 **3~4분 동안 아무 일도 안 일어나는 것처럼 보인다.** 화면은
`ssh_wait` 을 "접속 기다리는 중" 이라고만 말하는데, 실제로 그 시간을 쓰는 것은 접속이
아니라 **스택 설치**다(slim 은 여기서 torch 를 받는다). 서버 로그에는
`학습 스택 설치 중: …` 이 찍히고 있었는데 화면엔 안 갔다 — 그래서 사람은 접속 문제를
의심하게 된다.

`CloudJob.note` 를 뒀다. `reason`(왜 끝났나)과 다른 자리다 — 이건 **지금 무엇을
기다리는지**다.

⚠ **로그 꼬리를 그대로 올리지 않는다.** `bootstrap.sh` 의 출력에는 로케일 경고와 pip
막대가 섞여서, 그대로 보내면 화면이 `bash: warning: setlocale…` 을 "진행 상황" 으로
보여 준다 — §12-13 에서 **로그가 정확히 그랬던** 실수를 화면에서 반복하는 셈이다.
`install_note()` 가 `== torch 2.11.0 (cu126)` 같은 절 제목이나 `Downloading …` 만 고른다.

⚠ **칸이 바뀌면 메모를 비운다.** 안 비우면 학습이 시작된 뒤에도 "torch 받는 중" 이 남아
거짓말이 된다. `set_phase()` 가 지운다.

#### 이 회차로 확인한 것

같은 시각 실기(인스턴스 51389578, `piper-box-8404`, $0.0775/h):

```
0분    phase=ssh_wait   학습: idle   runner=systemd   ← "준비 중" 구간
6.8분  phase=training   학습: running runner=ssh      step 1000/20000
```

즉 진행 창이 없던 게 아니라 **그 구간이 길었고 화면이 이유를 말하지 않았다.** 학습이
시작되면 로그·로스 곡선은 로컬과 **같은 자리**에 그대로 나온다 — 러너만 갈아끼우는
구조라 위쪽은 로그가 어디서 왔는지 모른다.


### 12-25. 중간 체크포인트는 Hub 로 못 받는다 (2026-09-18)

20K 학습에 `save_freq=5000` 을 줬는데 결과에 최종본 하나만 왔다. 확인해 보니 인자는
제대로 걸렸고(`--save_freq=5000`), 기계 안에는 네 벌이 찍혔을 것이다. 안 온 이유는
**가져오는 쪽 둘 다 최종본만 보고 있었기** 때문이다:

| 경로 | 가져오는 것 | 왜 |
|---|---|---|
| Hub | 최종 모델 **9개 파일** | `push_to_hub` 가 학습 끝에 **한 번만** 올린다(§12-4) — 중간 것은 애초에 Hub 에 안 간다. 우리 선택이 아니다 |
| `scp` 보험 | `checkpoints/last/pretrained_model` 하나 | 이건 우리가 그렇게 정했다 |

실측으로 `wego-hansu/cloud_test1` 저장소를 열어 보니 9개 파일(206.7MB)뿐이었다.

#### 붙인 것: 선택 스위치

기본은 **끈 상태**다 — 최종본만 오는 것이 싸고 대부분은 그것으로 족하다. 켜면 기계가
**살아 있는 동안** `checkpoints/*/pretrained_model` 을 전부 `scp` 로 끌어온다.
실측 기준 한 벌 200MB · 나가는 트래픽 $0.017/GB 라 네 벌이면 **약 $0.014** 다.

⚠ **순서가 전부다.** Hub 냐 `scp` 냐를 가른 **뒤**에 하면 늦는다 — Hub 경로로 끝난
회차는 파기가 먼저 오고, 그런데 중간 체크포인트는 **기계에만** 있다. 그래서 판정보다
앞에 둔다. 테스트가 그 순서를 고정한다.

⚠ **놓는 자리는 스캐너가 이미 아는 모양**이다
(`<run>/checkpoints/<step>/pretrained_model/`, `model_scanner._scan_train_outputs`).
새 형식을 만들면 받아 놓고 화면에 안 뜬다 — 회수 경로에서 한 번 겪은 함정이다.

⚠ **`last` 는 건너뛴다.** 최종본은 Hub 든 `scp` 든 이미 받는다 — 또 받으면 같은
200MB 를 두 번 낸다.

⚠ **덤이 본체를 막지 않는다.** 체크포인트 회수가 실패해도 최종본 회수와 파기는 그대로
간다. 이것 때문에 파기가 막히면 본전도 못 찾는다.

#### ⚠ 이미 사라진 것은 못 되살린다

이번 회차의 기계는 이미 파기됐고 중간 체크포인트는 같이 사라졌다. 분리된 흐름에서는
학습이 끝나도 기계가 남으므로 **파기 전까지는** 거기 있다 — 화면이 그 사실을 말해 주지
않는 점은 §12-28 에서 고쳤다.


### 12-26. 받는 순간부터 로컬 학습과 같은 폴더로 (2026-09-21)

「클라우드 GPU에서 학습한거 다운 받을 때부터 로컬에서 한거랑 똑같은 폴더 구조로 할 수
있게 해야 될텐데」 — 안 돼 있었다. 화면에서 묶는 기획을 하던 중에 받은 지적이고, 맞다:
**애초에 같은 모양으로 받으면 묶을 일이 없다.**

#### 갈라져 있던 모양 (실측)

| | 자리 |
|---|---|
| 로컬 학습 | `~/outputs/train/2026-09-18/09-52-01_act/checkpoints/{020000, last→020000}/pretrained_model/` |
| 클라우드 중간 | `<models_dir>/w5-split-act/checkpoints/005000/pretrained_model/` |
| 클라우드 최종 | `<models_dir>/models--wego-hansu--w5-split-act/snapshots/<hash>/` |

셋 다 스캐너가 읽기는 한다(§12-10 이 본 것은 여기까지다). 그런데 **한 번의 학습 결과가
화면에서 두 덩어리로 갈라지고**, 최종본은 학습이 아니라 "직접 받은 모델" 로 읽혔다.

#### 이름은 우리가 지을 것이 아니었다

원격도 같은 lerobot 이라 **이미 같은 이름을 쓰고 있었다**:

```
/root/outputs/train/2026-09-18/02-13-23_act/checkpoints/020000/pretrained_model
```

- `scp` 경로는 이 전체 경로를 이미 손에 들고 있었다 — 옛 코드가 `p.rsplit("/", 2)[1]` 로
  **step 만 떼고 앞을 버렸다.**
- Hub 경로는 물어볼 데가 없어 보였는데, 실측해 보니 **올라간 `train_config.json` 안에
  주소가 들어 있다**: `output_dir: outputs/train/2026-09-18/02-13-23_act`. 그래서 기계가
  이미 파기됐거나 크래시로 죽었어도 원래 이름을 복원할 수 있다.

제로패딩(`020000`)도 lerobot 이 붙인 것이다. 우리가 `20000` 으로 고쳐 쓰면 그 순간
로컬과 갈린다.
...
| 자리 | 전 | 후 |
|---|---|---|
| `layout.py` (새로) | — | 자리를 아는 유일한 모듈. 뿌리·학습 이름·체크포인트 경로 |
| `rescue.dest_for()` | `models_dir/<repo 이름>/checkpoints/<step>` | `<뿌리>/<원격 run>/checkpoints/<step>/pretrained_model` |
| `retrieve.pull()` | `snapshot_download(cache_dir=…)` | `local_dir` 로 임시 수신 → `train_config.json` 읽고 이사 |
| `retrieve.local_snapshot()` | HF 캐시 모양만 | 새 자리 **먼저**, 없으면 옛 캐시도 본다 |

받고 나면 스캐너가 이렇게 읽는다(실측, 진짜 `config.json` 으로 확인):

```
id=2026-09-18/02-13-23_act/005000   run=2026-09-18/02-13-23_act  step=5000   type=act
id=2026-09-18/02-13-23_act/010000   run=2026-09-18/02-13-23_act  step=10000  type=act
id=2026-09-18/02-13-23_act/last     run=2026-09-18/02-13-23_act  step=None   type=act
```

학습 묶음 **1개**. 로컬 학습과 글자 하나까지 같은 모양이라 화면에서 구분할 필요 자체가
없어졌다.

#### ⚠ `last` 만 "다 왔다" 로 친다

중간 체크포인트도 같은 저장소를 가리킨다. 아무거나 맞다고 하면 `scp` 로 중간만 받아 둔
회차에서 **최종본을 안 받고 다 됐다고 말한다.** 테스트가 이걸 못 박는다.

#### ⚠ `local_dir` 이지 `cache_dir` 이 아니다

`cache_dir` 은 blob + 심볼릭 구조를 만든다 — 옮기면 링크가 깨지고, 복사하면 같은 200MB 를
두 벌 갖는다. `local_dir` 은 진짜 파일을 놓지만 옆에 자기 메타(`.cache/huggingface/`)를
남기므로 옮기기 전에 지운다.

#### 남는 대가 (알고 넘어간다)

1. **이미 받아 둔 것은 옛 자리에 그대로다.** 자동 이사는 안 한다 — 옛 캐시도 계속 스캔되니
   사라지지는 않고, 지우는 판단은 사람 몫이다.
2. **저장소 페이지에서 같은 repo 를 [다운로드] 하면 사본이 하나 더 생긴다.** 전에는 자리가
   같아 겹쳤고, 이제는 진짜 두 벌(200MB)이 된다.
3. **`last` 가 목록에 한 줄 더 만든다.** 로컬 학습도 이미 그렇다(실측: 체크포인트 하나짜리
   학습이 항목 2개) — 로컬과 같아진다는 요구는 이게 맞고, 중복 표시는 별개 문제다.

| | |
|---|---|
| 테스트 | `test_cloud_rescue.py` 13 · `test_cloud_retrieve.py` 42 · 전체 **2274 통과** |
| 실측 확인 | 진짜 `train_config.json`(`wego-hansu/w5-split-act`) 으로 `_scan_train_outputs` 까지 |

### 12-27. 적어 두고 안 한 한 줄 — 배포판은 나갈 ssh 가 없었다 (2026-09-21)

「120번은 tmux 없이 설치했는데 잘 쓰고 있지 않나?」 — 맞는 지적이었고, 덕분에 범위가
제대로 줄었다.

#### 셋을 한 줄로 묶어 둔 것이 화근이었다

§3-4 의 표가 "`ssh`·`tmux`·`vastai` 가 없다" 로 셋을 묶어 놨는데, 셋은 **있어야 하는 쪽이
다르다**:

| 바이너리 | 있어야 하는 쪽 | 상태 |
|---|---|---|
| `tmux` | **빌린 기계** — `command -v tmux` 는 ssh 로 원격에서 돈다(`runners/ssh.py`) | 학습 이미지에 ☑ (처음부터) |
| `ssh`·`scp` | **게이트웨이**(나가는 쪽) | ☒ → 이번에 넣었다 |
| `vastai` | 게이트웨이 | ☑ (그새 들어갔다) |

#### 왜 여태 안 걸렸나

`train_ssh_host` 가 비면 학습은 **systemd 러너**로 간다(`core/config.py`). 로컬 학습·수집·
추론은 ssh 를 아예 안 타므로 .120·.44 는 멀쩡히 돌았다. 개발 머신은 게이트웨이가 컨테이너가
아니라 저장소에서 직접 돌아(`ExecStart=… -m uvicorn`, WorkingDirectory=저장소) 호스트의
ssh 를 썼다 — 클라우드 학습을 전부 여기서 돌린 이유다.

⚠ **그런데 `vastai` 는 들어 있었다.** `/readiness` 는 `vastai`·API 키·SSH 키 셋만 보므로
배포 호스트에서도 **초록불**이 뜬다. 빌리기가 열리고, 기계가 만들어지고, 그 다음에야
`ssh 가 없습니다` 로 끝난다 — 셋 다 없던 때보다 나쁜 조합이다.

#### 넣은 자리: 베이스의 **끝**

apt 목록은 `Dockerfile.base` 39행인데 거기 끼우면 뒤의 torch·lerobot 계층이 전부 새
다이제스트가 되어 **호스트가 수 GB 를 다시 받는다**. 파일 끝에 새 `RUN` 으로 붙이면 앞
계층이 그대로라 새로 나가는 것은 그 한 층뿐이다 — `grpcio`·`torchmetrics` 가 들어간 방식과
같다. 태그는 `cu130-3 → cu130-4`.

| | |
|---|---|
| 빌드 | 베이스 2.2초(새 계층만) · 12GB 그대로 |
| 확인 | `piper-web-backend:latest` 안에 `ssh`/`scp` ☑ · `tmux` 없음(맞다) · OpenSSH 9.2p1 |
| 테스트 | `test_release.py::test_the_gateway_image_can_reach_out_over_ssh` — 계획이 적어만 뒀던 그 테스트 |

⚠ **`openssh-server` 가 아니다.** 이 컨테이너로 들어오는 접속은 없다. 테스트가 둘을 같이
못 박는다(클라이언트는 있어야 하고, 서버는 없어야 한다).

#### 점검 목록에도 넣었다

베이스가 바뀌었어도 **릴리스를 받아야** .120·.44 가 고쳐진다. 그전까지 그 두 대는
여전히 ssh 없이 도는데, 점검이 넷만 보면 계속 초록불이다. 그래서 `/readiness` 에
`ssh` 항목을 넣었다 — 이제 **빌리기 전에** 막히고, 화면은 "새 릴리스를 받아 주세요"
라고 말한다(사용자가 컨테이너에 ssh 를 깔 방법은 없으므로 사용자 탓으로 적지 않는다).

⚠ `scp` 도 같이 본다. 회수가 그걸 직접 부르고, 거기서 실패하면 잃는 것은 접속이 아니라
**학습 결과**다.

실측으로 판정이 갈리는 것까지 확인했다:

```
cu130-3  ssh_ok=False     ← 지금 .120·.44 가 도는 베이스
cu130-4  ssh_ok=True
```

| | |
|---|---|
| 테스트 | 기준선(다 있으면 초록) · ssh 없으면 `ready=False` · **`scp` 만 없어도** 빨간불 |
| 전체 | **2286 통과** · 프론트 빌드 ☑ (`ssh: 'ssh 클라이언트'` 라벨) |

### 12-28. 파기 창이 무엇을 잃는지 센다 (2026-09-21)

§12-25 가 남겨 둔 자리다. 창은 "회수하지 않은 체크포인트가 **있으면** 지금 잃습니다"
라고만 했는데, 있는지 없는지를 사람이 알 길이 없었다 — 중간 체크포인트는 Hub 에 아예
안 올라가므로(푸시는 학습 끝에 한 번뿐) 틀리면 되돌릴 수도 없다.

`GET /instances/{id}/checkpoints` 가 기계에 직접 물어보고, 창이 열릴 때 채운다.

#### ⚠ 세 상태를 섞지 않는다

| 상태 | 화면 |
|---|---|
| 아직 묻는 중 | "중간 체크포인트를 확인하는 중…" (회색) |
| 못 물어봄 | "**확인하지 못했습니다** — … 있는지 모르는 채로 파기하게 됩니다" (주황) |
| 물어봄 · N개 | "회수하지 않은 중간 체크포인트 **N개**가 있습니다 (step 005000, 010000)" (빨강) |
| 물어봄 · 0개 | "중간 체크포인트는 없습니다" (회색) |

**못 물어본 것을 "0개" 로 뭉개는 것이 이 창에서 제일 비싼 거짓말이다** — 사람은 없다고
믿고 지우고, 그걸로 끝이다. §12-23 의 SSH 키 판정에서 이미 겪은 모양이라 같은 규칙을
그대로 적용했다.

#### 실패를 **반대로** 다루는 함수 둘

`list_checkpoints()` 를 `fetch_checkpoints()` 에서 떼어냈다. 같은 `ls` 를 돌리지만 실패를
정반대로 다룬다:

| | 실패하면 | 왜 |
|---|---|---|
| `fetch_checkpoints` | 삼키고 `[]` | 덤이 파기를 막으면 기계가 계속 돈다 |
| `list_checkpoints` | **올린다** | 부르는 쪽이 "0개" 와 "못 물어봤다" 를 갈라야 한다 |

테스트가 양쪽을 같이 못 박는다 — 하나는 `pytest.raises`, 하나는 `== []`.

⚠ **조회가 파기를 막지 않는다.** 창은 바로 뜨고 숫자만 나중에 채워진다. 조회가 느리다고
버튼을 잠그면, 정작 급할 때(요금이 나가는 중) 못 끄게 된다.

| | |
|---|---|
| 테스트 | 개수·step 이름 그대로 · 접속 실패는 `reachable=false` · 주소 없으면 "모름" · 없는 기계는 404 · 목록/회수의 실패 처리 대비 |
| 전체 | **2293 통과** · 프론트 빌드 ☑ · 실물 404 확인 |
