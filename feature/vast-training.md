...
⚠ **"이 기계" 라는 라벨을 화면이 지어내지 않는다.** 러너는 서버 설정
(`PIPER_TRAIN_SSH_HOST`)이 정하므로 사내 SSH 박스일 수도 있다 — `/training/status` 의
`runner` 를 읽어 "사내 서버 (SSH)" 로 바꿔 단다. 화면이 "로컬" 이라 적어 두고 실제로는
사내 서버에서 도는 것이 제일 나쁘다.
