...
#### ⚠ 곁가지: `too_old` 는 지금 **뜰 수 없다**

`KNOWN_CC` 의 최솟값이 500 이고 `CC_MIN` 도 500 이라, `compute_cap < CC_MIN` 인
**알려진** 값이 없다. 즉 `gpu_support()` 의 `too_old` 가지는 현재 도달 불가다. 틀린
동작은 아니지만(모르는 값은 `unknown` 으로 가는 것이 의도다), 나중에 `KNOWN_CC` 에
더 낮은 값을 넣기 전에는 그 문구가 화면에 안 나온다는 뜻이다.
