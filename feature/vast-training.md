...
#### ⚠ 확인하다 태운 것: slim 템플릿으로는 학습이 안 된다

확인용으로 **slim** 템플릿을 골라 돌렸더니, 기계를 빌리고 이미지를 받고 SSH 까지 붙은
**뒤에** 3초 만에 죽었다:

```
/opt/venv/bin/python: No module named 'lerobot'
```

로컬에서 이미지를 열어 확인했다 — **slim(1.32GB)에는 lerobot 이 아예 없다**
(`lerobot_policy_act_aux` 뿐). full 은 14GB 에 lerobot 0.5.0 + torch 2.11.0+cu126 이다.
즉 slim 은 학습용이 아니라 **추론/정책서버용**이다.

화면은 기본값을 full 로 고르고 있었지만 slim 도 **고를 수 있었다.** 고르면 94초와
$0.0145 를 쓰고 나서야 알게 된다. 그래서 둘을 붙였다:

- `Template.can_train` — 판정은 **백엔드**가 한다(화면이 이름으로 짐작하면 어긋난다)
- `POST /rent` 가 **빌리기 전에** 400 으로 거절한다 — 돈이 나가기 전에 아는 것과
  나간 뒤에 아는 것의 차이다

⚠ 판정은 **"안 되는 것을 안다" 쪽만** 한다. 모르는 변종(사용자가 만든 템플릿)까지
막으면 쓸 수 있는 것을 못 쓰게 되고, 템플릿 조회가 실패해도 빌리기는 통과시킨다 —
Vast 가 안 닿는다고 빌리기를 막을 이유는 없다.

⚠ 그리고 **못 쓰는 것을 목록에서 숨기지 않는다.** 추론용으로는 쓸 것이고, 없는 셈
치면 "왜 하나뿐이지" 가 된다. 대신 왜 못 쓰는지 적는다.
