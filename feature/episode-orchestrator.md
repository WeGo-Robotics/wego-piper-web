...
## 정책 종류에 따라 판단이 닿는 길이 다르다

루프는 판단을 `task` 문장으로 내보낸다. **VLA 는 그 문장을 읽지만 ACT 는 안 읽는다**
(`core/policies.takes_language`). 그래서 ACT 를 올려둔 채 루프를 돌리면 LLM 이 매
회차 다른 결정을 내려도 로봇은 같은 동작만 반복하고, 저널에는 결정이 꼬박꼬박
쌓인다 — 에러도 경고도 없이. **돌아가는 것처럼 보이는 실패**라 가장 나쁘다.

|  | VLA (언어 정책) | ACT 등 |
|---|---|---|
| LLM 출력 | 문장 (자유) | 스킬 이름 (레지스트리에서 고름) |
| 전달 | 버스 파라미터 (`task`) | 체크포인트 교체 |
| 비용 | 0 — 재시작 없음 | 추론 재시작 (수 초) |
| 조합 | 정책 하나가 다 처리 | 스킬마다 가중치 하나 |

마지막 줄이 설계를 가른다. 판단이 `(target, destination)` 두 축이라, ACT 를
목적지별로 학습하면 N개, 조합별이면 N×M개가 필요하다.

### 단계

1. ☑ **침묵 제거** — 시작 시 `_require_actionable_policy()` 가 "판단이 로봇에 닿는가"를
   본다. 언어 정책이 아니고 스킬도 없으면 **시작을 거부**한다. 정책을 확인할 수
   없으면(직접 편집한 CLI) 안 읽는 쪽으로 본다 — 모를 때 낙관하면 조용히 헛돈다.
2. ☑ **스킬 레지스트리** — `OrchestratorConfig.skills = {이름: 체크포인트}`.
   판단 스키마에 `skill` 슬롯이 붙고, **허용값은 레지스트리에서 런타임에** 들어간다.
   `Field(description=...)` 이 그대로 guided decoding 에 실리므로 목록을 코드에
   박으면 가중치를 추가할 때마다 여기를 고쳐야 하고, 안 고치면 LLM 이 없는 스킬을
   고른다.
3. ☐ **교체 실행** — 스킬이 바뀔 때만 추론을 재시작한다. 같은 스킬이 이어지면
   그대로 둔다(연속으로 같은 것을 집을 때 재시작이 없다).
   전제: 추론 시작 로직이 라우터 밖에서도 불릴 수 있어야 한다 —
   지금은 `routers/models.py:start_inference` 안에 카메라·팔 준비까지 묶여 있다.
   그 추출은 **팔을 움직이는 경로**를 건드리므로 실기 확인이 필요하다.
