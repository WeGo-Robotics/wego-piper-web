...
#:   명령 50.9~65° 인데 피드백은 18~22° 에서 안 움직였고, 전류 10.3 A ·
#:   `stall` 86.9% · `driver_overcurrent` 86.9% 로 드라이버가 스스로 차단할
#:   때까지 6 초를 밀었다. **모터 온도가 65 → 87°C.** 플래그를 기록만 하고
#:   반응을 안 한 것이 구멍이었다 — 검사가 모터를 태울 수 있다.
#:
#: ⚠ **플래그가 서 있다고 지금 걸려 있는 것은 아니다.** 기구 한계에 한 번
#:   부딪히면 과전류·위치 오차가 **래치되어** 그 뒤로는 어느 방향으로도 반응하지
#:   않는다 — 그래서 "아래가 막혔다" 던 관절이 위로도 안 움직였다. 기구물을
#:   치워도 안 돌아온다. `0x150` 리셋(`Arm.clear_error`, 로봇 페이지의 리셋)이
#:   에러를 지워야 다시 움직인다. 중단 메시지가 그 길을 가리켜야 한다.
