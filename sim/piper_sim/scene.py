...
def load_model(spec: dict | None = None):
    """가상환경을 굽는다. `spec=None` 이면 기본 가상환경(`assets/default_scene.json`).

    ⚠ 예전엔 `piper_scene.xml` 을 **그대로** 열었다. 지금 그 파일은 **바탕**이다 —
    팔·테이블·카메라·조명처럼 릴리스가 정하는 것만 담고, 테이블 위 사물은 가상환경 JSON 이
    `scene_spec.compose` 로 얹는다 (feature/sim-scene-editor.md). 기본 가상환경이 옛 XML 과
...
class JointMap:
    ...
    def __init__(self, model) -> None:
        ...
        #: 가상환경의 물체 — id → body id. **월드의 직계 자식에서 팔 뿌리만 뺀 것**이다.
        #: 가상환경 JSON 이 물체를 월드 바로 밑에 얹으므로(`scene_spec.compose`) 이 규칙 하나로
        ...
        #: 옛 이름 — 기본 가상환경의 큐브. 없는 가상환경이면 `-1`(부르는 쪽이 확인한다).
