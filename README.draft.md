...
<img src="docs/images/study.jpg" alt="학습 — 손실 곡선, 메트릭, 로그">

손실 곡선과 로그가 도는 동안 화면에 온다. GPU 가 없거나 모자라면 **클라우드 GPU 를 빌려**
같은 화면에서 학습을 건다 — 빌리고, 올리고, 학습하고, 가중치를 도로 가져오고, 기계를 끄는
것까지. 빌려 둔 기계가 놀고 있으면 **지금까지 얼마 나갔는지와 함께** 경고한다.

<!-- 필요: 클라우드 GPU 화면 (빌린 기계 목록 + 학습이 도는 중) → docs/images/cloud.jpg -->
