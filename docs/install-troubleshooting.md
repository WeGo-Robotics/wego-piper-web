...
| 2절에서 pip 가 `Package 'piper-bus' requires a different Python: 3.10.x not in '>=3.11'` | 22.04 의 시스템 파이썬은 3.10 인데 v0.4.15 이하 wheel 이 `>=3.11` 을 **선언**했다 — 코드는 3.10 에서 돈다(실측) | v0.4.16 부터 하한이 3.10. 그때까지: `~/.venvs/piper-daemons/bin/pip install --no-deps --force-reinstall --ignore-requires-python ~/piper-web-deploy/<버전>/wheels/*-<버전>-*.whl` 뒤 다시 `./piper-install.sh`. 3.10 미만(20.04)은 OS 를 올린다 |
| 조종·추론 명령이 팔에 안 간다 — 큐브는 옮겨지고 카메라도 나오는데. simd/robotd 저널에 `Permission denied: '/dev/shm/piper.arm.<팔>.action'` 트레이스백 | 팔로워 action 세그먼트를 게이트웨이 **컨테이너(root)** 가 0600 으로 만들어 호스트 데몬(일반 사용자)이 못 열고, 그 예외로 명령 스레드가 조용히 죽었다(v0.4.16 이하) | **v0.4.17 로 올린다** — 0644 로 만들고 데몬은 열기 실패를 재시도한다. ⚠ 우회로 호스트 사용자가 파일을 먼저 만들어 두는 건 **안 된다**: `fs.protected_regular`(우분투 기본 2)가 `/dev/shm` 같은 sticky 디렉토리에서 남의 파일을 `O_CREAT` 로 여는 것을 root 에게도 거부해 릴레이 쪽이 죽는다 |
| 웹 [업데이트]가 `unitd.update: 받기 스크립트가 없습니다: …/piper-web-deploy/v*/piper-install.sh — 이 번들이 낡았다` | README 대로 **버전 없이** 깔면 v0.4.17 이하 `piper-install.sh` 가 번들을 `latest/` 에 풀었는데, unitd 는 `v*/` 만 봤다(문구도 엉뚱했다) | 터미널에서 `./piper-install.sh` 를 한 번 — v0.4.18 부터는 `latest` 로 받아도 매니페스트 버전 이름 디렉토리에 풀고(`latest` 는 링크), unitd 도 옛 `latest/` 를 읽는다. 그 뒤로는 웹에서 된다 |
