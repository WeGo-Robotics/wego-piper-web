...
3. ☑ **ZMQ 브리지 3개를 Redis로 교체** — 프로세스 경계는 그대로 두고 전송만 바꿨다.
   `5555` 파라미터 → 리스트 큐, `5557` 녹화 제어 → 리스트 큐,
   `5556` 프리뷰 → **큐가 아니라 TTL 키**(최신 프레임만 의미가 있어 큐면 무한히 쌓인다).
   주소 3개가 `PIPER_REDIS_URL` 하나로 줄었고, `ProcessManager` 가 모든 자식에게 넣는다.
   pub/sub 이 아닌 이유: 구독자가 없으면 조용히 버려서 정책 로딩 중에 보낸 값이 사라진다.
   ⚠ **ZMQ 는 소켓을 닫으면 큐도 사라졌지만 Redis 리스트는 남는다** — 세션 시작·종료 때
   비우고, 그 비우기가 wrapper 기동보다 **먼저**여야 한다 (테스트로 잠갔다).
