...
5. ☑ 컨테이너에서 `privileged`/`/dev`/`/run/udev`/`/lib/modules` 제거, `ipc: host` 추가.
   ...
   `network_mode: host` 도 뺐다 — Redis 는 호스트 유닉스 소켓 마운트로,
   nginx 는 브리지의 서비스 이름(`backend:8000`)으로 해결했다.
   **남은 것은 GPU + `ipc: host` 뿐이다** (문서 "얻는 것" #2 의 목표 상태).

   **실기 확인**: `docker compose up` → 컨테이너가 장치를 하나도 안 열고
   카메라(rsd)·팔(robotd)을 다 본다. 호스트 요구사항이 하나 늘었다 —
   redis.conf 의 `unixsocket` 은 기본값이 주석 처리돼 있어 켜야 한다.
