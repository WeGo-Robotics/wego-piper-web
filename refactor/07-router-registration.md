...
> `ROUTERS` 리스트 + 루프. `/openapi.json` 경로 **149개가 변경 전후 완전히 동일**함을 확인했다.
> `pkgutil` 자동 순회는 문서 권고대로 쓰지 않았다 (등록 순서가 암묵적이 되고 import 부작용이 숨는다).
> 대신 `app/routers/` 의 모듈이 `ROUTERS` 에 다 있는지 테스트로 잡는다.
