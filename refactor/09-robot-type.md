# 9. `robot_type: 'piper_follower'` 5곳 하드코딩 (C급) — ☑ 완료

> **(a) 프론트에서 제거** 채택. 5곳이 지워지고 정본이 `robot_manager.selected_type` 하나로 모였다.
> 이건 정리만이 아니라 **버그 수정이기도 하다** — 로봇 페이지에서 다른 기종을 선택해도
> 프론트가 보낸 `'piper_follower'` 가 이겨서 선택이 무시되고 있었다.
